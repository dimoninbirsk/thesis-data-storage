...
def fetch_rule_details(rule_key):
    ...
    """
    print(f"DEBUG: fetch_rule_details: Fetching details for rule_key: {rule_key}")  # LOG
    try:
        ...
        print(f"DEBUG: fetch_rule_details: Found rule: {rule.get('name', 'No')}")  # LOG
        return {
            "name": rule.get("name", "No"),
            "description": rule.get("htmlDesc", "No"),
        }
    except requests.exceptions.RequestException as e:
        print(f"ERROR: fetch_rule_details: Error fetching rule details for {rule_key}: {e}")
        return {"name": rule_key, "description": "Not Found"}
    except (KeyError, IndexError, json.JSONDecodeError) as e:
        print(f"ERROR: fetch_rule_details: Error processing response for rule {rule_key}: {e}")
        return {"name": rule_key, "description": "Not Found"}
...
def fetch_project_issues(project_key):
    ...
    """
    print(f"DEBUG: fetch_project_issues: Fetching issues for project: {project_key}")  # LOG
    issues = []
    ...
    try:
        for issue_type in issue_types:
            page = 1
            while True:
                print(f"DEBUG: fetch_project_issues: Fetching {issue_type} page {page}")  # LOG
                response = requests.get(
                ...
                print(f"DEBUG: fetch_project_issues: Got {len(new_issues)} issues of type {issue_type}")  # LOG
                if len(new_issues) < 100:
                    break
                page += 1
    except requests.exceptions.RequestException as e:
        print(f"ERROR: fetch_project_issues: Error fetching issues for project {project_key}: {e}")
    except json.JSONDecodeError as e:
        print(f"ERROR: fetch_project_issues: Error processing JSON response for project {project_key}: {e}")
    print(f"DEBUG: fetch_project_issues: Found {len(issues)} issues in total")  # LOG
    return issues
...
def save_data(project_key, issues):
    ...
    """
    print(f"DEBUG: save_data: Saving data for project: {project_key}, {len(issues)} issues") # LOG
    rule_details_cache = {}
    processed_issues = []
    for issue in issues:
        rule_key = issue.get("rule")
        print(f"DEBUG: save_data: Processing issue with rule key: {rule_key}") # LOG
    ...
    print(f"DEBUG: save_data: Saving JSON to {json_path}") # LOG
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(processed_issues, f, indent=2, ensure_ascii=False)
    csv_path = f"{OUTPUT_DIR}/{project_key}_issues.csv"
    print(f"DEBUG: save_data: Saving CSV to {csv_path}") # LOG
    df = pd.DataFrame(processed_issues)
    df.to_csv(csv_path, index=False, encoding="utf-8-sig")
    print(f"DEBUG: save_data: Saved {len(processed_issues)} issues.") # LOG
...
def analyze_project_issues(issues):
    ...
    """
    print(f"DEBUG: analyze_project_issues: Analyzing {len(issues)} issues") # LOG
    statistics = {
    ...
    print(f"DEBUG: analyze_project_issues: Statistics generated: {statistics}") # LOG
    return statistics
...
def sort_and_limit_by_value(data, limit: int = 10):
    ...
    """
    print(f"DEBUG: sort_and_limit_by_value: Sorting and limiting data, limit={limit}") # LOG
...
def analyze_all_projects(project_statistics):
    ...
    """
    print(f"DEBUG: analyze_all_projects: Analyzing statistics for all projects") # LOG
    all_statistics = {
        "by_type": defaultdict(int),
        "by_severity": defaultdict(int),
        "by_rule": defaultdict(int),
    ...
    for project, stats in project_statistics.items():
        print(f"DEBUG: analyze_all_projects: Aggregating stats for {project}") # LOG
        for key in all_statistics.keys():
            for item, count in stats[key].items():
                all_statistics[key][item] += count

    print(f"DEBUG: analyze_all_projects: Aggregated statistics: {all_statistics}") # LOG
    return all_statistics


def generate_combined_report(all_issues):
    """Generates the combined report."""
    print("DEBUG: generate_combined_report: Starting combined report generation")
    ...
    print(f"DEBUG: generate_combined_report: Combined report saved to: {csv_file_path}")
    ...
    print(f"DEBUG: generate_combined_report: Combined report saved to: {excel_file_path}")
    ...
    print(f"DEBUG: generate_combined_report: Combined report saved to: {os.path.join(OUTPUT_DIR, 'combined_report.json')}")
...
def main():
    ...
    """
    print("DEBUG: main: Starting the script") # LOG
    project_statistics = {}  # Collect statistics for all projects

    # Create main output dir if it doesnt exist
    if not os.path.exists(OUTPUT_DIR):
        os.makedirs(OUTPUT_DIR)
        print(f"DEBUG: main: Created output directory: {OUTPUT_DIR}") # LOG
    else:
        print(f"DEBUG: main: Output directory already exists: {OUTPUT_DIR}") # LOG

    # Prepare a list to store all issues for the combined report
    all_issues = []

    for project in PROJECT_KEYS:  # Убрали tqdm
        print(f"DEBUG: main: Processing project: {project}") # LOG
        # Create a directory for each project, before all else
        project_dir = os.path.join(OUTPUT_DIR, project)
        if not os.path.exists(project_dir):
            os.makedirs(project_dir)
            print(f"DEBUG: main: Created project directory: {project_dir}") # LOG
        else:
            print(f"DEBUG: main: Project directory already exists: {project_dir}") # LOG

        issues = fetch_project_issues(project)
        if issues:
            count = save_data(project, issues)
            print(f"{project}: saved {count} issues")
            ...
            try:
                ...
                print(f"ERROR: main: File {OUTPUT_DIR}/{project}_issues.json not found.")
                ...
                print(
                    f"ERROR: main: Error reading JSON from file {OUTPUT_DIR}/{project}_issues.json."
                )
                project_statistics[project] = {
                    "by_type": {},
                    "by_severity": {},
    ...
    generate_combined_report(all_issues)
